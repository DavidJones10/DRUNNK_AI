...
function MakeKnob(imgSrc, diameter, locx, locy, lowNum, hiNum, defaultNum, numPlaces, label) {
  ...
  this.update = function() {
    ...
    if (dist(this.pos.x, this.pos.y, mouseX, mouseY) < this.diameter/2) {
      ...
      this.rotateMe=this.currentRot+map(mouseY, this.myY, mouseY+200, 0, 280);
   ...
   // add the display value and label
    ...
    this.displayValue = nfc(this.knobValue, numPlaces); // added in version 4.2.1
  	text(this.displayValue, this.pos.x, this.pos.y+this.diameter/2+this.textPt); // display value to 2 decimal places
  	fill(this.textColor);
  	text(this.label, this.pos.x, this.pos.y+this.diameter/2+2.4*this.textPt-3);
    // set the cursor
