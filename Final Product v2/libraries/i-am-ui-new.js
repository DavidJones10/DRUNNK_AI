...
function button(p, x, y, w, h, r) {
  if (!r) p.rect(x,y,w,h,5)
  
  if (hitbox(x, y, w, h, p.mouseX, p.mouseY)) {
    p.fill("#dddddd44");
    p.rect(x, y, w, h, r||5);
    p.cursor("hand");
  }
  ...
  if (
    hitbox(x, y, w, h, p.mouseX, p.mouseY) &&
    p.mouseIsPressed &&
    pdown != p.mouseIsPressed
  ) {
    return true;
...
function draggable(p,v,w,h) {
  if (hitbox(v.x, v.y, w, h, p.mouseX, p.mouseY) && 
      !umouse && p.mouseIsPressed && p.mouseIsPressed != umouse) {
    p.cursor('grabbing');
    v.drive = true
  }
  if (!p.mouseIsPressed) v.drive = false
  if (v.drive) {
    umouse = true
    
    v.x -= p.pmouseX-p.mouseX
    v.y -= p.pmouseY-p.mouseY
  }
...
function slider(p, v, x, y, w, m, f,r,g) {
  let ux = p.map(v, m, f, x, x+w);

  if (!r) {p.noFill();
  p.line(x, y, ux - 5, y);
  p.line(x + w, y, ux + 5, y);
  p.fill(255,0,0);
  p.rect(ux - 5,y-7.5, 10, 15, 5);}

  if (hitbox(x, y - 10, w, 20, p.mouseX, p.mouseY)) {
    p.cursor("grab")
    if (p.mouseIsPressed) {
      p.cursor("grabbing")
      if (p.keyIsDown(17) && g) {
        return clamp(gridlock(p.map(p.mouseX, x, x + w, m, f),g),m,f);
      }
      return clamp(p.map(p.mouseX, x, x + w, m, f),m,f);
    }
  }
